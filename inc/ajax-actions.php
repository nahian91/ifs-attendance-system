...
function ifs_erp_master_action_dispatcher() {
    ...
    if (isset($_POST['ifs_frontend_login']) && wp_verify_nonce($_POST['_wpnonce'] ?? '', 'ifs_front_login_nonce')) {
        ...
        $user_cap   = isset($_POST['login_captcha']) ? intval($_POST['login_captcha']) : 0;
        $actual_cap = isset($_SESSION['ifs_captcha_sum']) ? intval($_SESSION['ifs_captcha_sum']) : -1;

        if ($user_cap !== $actual_cap) {
            wp_safe_redirect(add_query_arg('ifs_err', 'captcha_invalid', wp_get_referer() ?: home_url()));
            exit;
        }

        $student = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}ifs_students WHERE email = %s AND status = 'active'",
            $email
        ));
        ...
        if ($student && wp_check_password($password, $student->password)) {
            ...
            wp_safe_redirect(remove_query_arg(['ifs_err'], wp_get_referer() ?: home_url()));
            exit;
        } else {
            wp_safe_redirect(add_query_arg('ifs_err', 'invalid_credentials', wp_get_referer() ?: home_url()));
            exit;
        }
    }
    ...
    if (isset($_GET['ifs_front_logout'])) {
        ...
        wp_safe_redirect(remove_query_arg(['ifs_front_logout'], wp_get_referer() ?: home_url()));
        exit;
    }

    // Guard: Only authenticated administrators/managers should trigger admin operations
    if (!is_user_logged_in() || !isset($_REQUEST['ifs_action'])) {
        return;
    }

    $action   = sanitize_text_field($_REQUEST['ifs_action']);
    $base_ref = wp_get_referer() ?: admin_url('admin.php?page=ifs-attendance');

    // 2. Add Student
    if ($action === 'add_student' && check_admin_referer('ifs_stu_nonce')) {
        if (!current_user_can('manage_options')) {
            wp_die(__('Unauthorized action.', 'ifs-attendance'));
        }
        ...
        $raw_pass    = $_POST['student_password'] ?? wp_generate_password(8, false);
        ...
        wp_safe_redirect(add_query_arg([
            'ifs_alert' => ($res ? 'stu_added' : 'stu_err'),
            'page_view' => 'students',
            'sub_tab'   => 'all'
        ], $base_ref));
        exit;
    ...
    if ($action === 'edit_student' && check_admin_referer('ifs_edit_stu_nonce')) {
        ...
        $data_to_update = [
            ...
            'institution'    => trim(sanitize_text_field($_POST['student_institution'] ?? '')),
            'batch'          => trim(sanitize_text_field($_POST['student_batch'] ?? '')),
            'status'         => trim(sanitize_text_field($_POST['student_status'] ?? 'active')),
            'photo_url'      => esc_url_raw($_POST['student_photo_url'] ?? '')
        ];
    ...
    if ($action === 'save_attendance' && check_admin_referer('ifs_att_save_nonce')) {
        ...
        $att_date = sanitize_text_field($_POST['attendance_date'] ?? current_time('Y-m-d'));
        ...
        $args = [
            ...
            'page_view'   => 'attendance'
        ];
        if ($b_filter !== '') {
            $args['filter_batch'] = $b_filter;
        }

        wp_safe_redirect(add_query_arg($args, $base_ref));
        exit;
    }
    ...
    if ($action === 'add_batch_entity' && check_admin_referer('ifs_batch_nonce')) {
        ...
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'batch_added', 'page_view' => 'batches'], $base_ref));
        exit;
    }
    ...
    if ($action === 'del_batch_entity' && check_admin_referer('ifs_del_batch_nonce')) {
        ...
        $wpdb->delete($wpdb->prefix . 'ifs_batches', ['id' => intval($_GET['batch_id'] ?? 0)], ['%d']);
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'batch_deleted', 'page_view' => 'batches'], $base_ref));
        exit;
    }

    // 8. Add & Delete Teacher
    if ($action === 'add_teacher_entity' && check_admin_referer('ifs_teacher_nonce')) {
        ...
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'teacher_added', 'page_view' => 'teachers'], $base_ref));
        exit;
    }
    ...
    if ($action === 'del_teacher_entity' && check_admin_referer('ifs_del_teacher_nonce')) {
        ...
        $wpdb->delete($wpdb->prefix . 'ifs_teachers', ['id' => intval($_GET['teacher_id'] ?? 0)], ['%d']);
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'teacher_deleted', 'page_view' => 'teachers'], $base_ref));
        exit;
    }

    // 9. Add & Delete Holiday
    if ($action === 'add_holiday_entity' && check_admin_referer('ifs_holiday_nonce')) {
        ...
        $wpdb->replace(
            $wpdb->prefix . 'ifs_holidays',
            [
                'holiday_date' => sanitize_text_field($_POST['holiday_date'] ?? ''),
                'title'        => sanitize_text_field($_POST['holiday_title'] ?? ''),
                'holiday_type' => sanitize_text_field($_POST['holiday_type'] ?? 'Official')
            ]
        );

        wp_safe_redirect(add_query_arg(['ifs_alert' => 'holiday_added', 'page_view' => 'schedule'], $base_ref));
        exit;
    }
    ...
    if ($action === 'del_holiday_entity' && check_admin_referer('ifs_del_holiday_nonce')) {
        ...
        $wpdb->delete($wpdb->prefix . 'ifs_holidays', ['id' => intval($_GET['holiday_id'] ?? 0)], ['%d']);
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'holiday_deleted', 'page_view' => 'schedule'], $base_ref));
        exit;
    }
    ...
    if ($action === 'add_setting_meta' && check_admin_referer('ifs_setting_nonce')) {
        ...
        if (!empty($meta_value)) {
            $wpdb->insert(
                $wpdb->prefix . 'ifs_meta',
                [
                    'meta_type'  => sanitize_text_field($_POST['meta_type'] ?? 'institution'),
                    'meta_value' => $meta_value
                ]
            );
        }

        wp_safe_redirect(add_query_arg(['ifs_alert' => 'meta_added', 'page_view' => 'settings', 'sub_tab' => 'institutions'], $base_ref));
        exit;
    }
    ...
    if ($action === 'del_setting_meta' && check_admin_referer('ifs_del_setting_nonce')) {
        ...
        $wpdb->delete($wpdb->prefix . 'ifs_meta', ['id' => intval($_GET['meta_id'] ?? 0)], ['%d']);
        wp_safe_redirect(add_query_arg(['ifs_alert' => 'meta_deleted', 'page_view' => 'settings', 'sub_tab' => 'institutions'], $base_ref));
        exit;
    }
    ...
    if ($action === 'save_general_settings' && check_admin_referer('ifs_gen_setting_nonce')) {
        ...
        update_option('ifs_portal_name', sanitize_text_field($_POST['ifs_portal_name'] ?? 'IFS Academic ERP'));
        update_option('ifs_receipt_prefix', sanitize_text_field($_POST['ifs_receipt_prefix'] ?? 'REC-'));

        wp_safe_redirect(add_query_arg(['ifs_alert' => 'settings_saved', 'page_view' => 'settings', 'sub_tab' => 'preferences'], $base_ref));
        exit;
    }
}